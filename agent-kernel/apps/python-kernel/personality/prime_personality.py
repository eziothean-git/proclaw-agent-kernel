...
class PrimePersonality:
    ...
    async def process_request(
        ...
        # Telemetry: Intent analysis start
        emit_telemetry(
            request_id=request.id,
            layer=3,
            layer_name="Prime Personality",
            component="PrimePersonality",
            operation="intent_analysis",
            status="start",
            message=f"Analyzing intent: {request.message[:50]}...",
            session_id=request.session_id,
        )
        ...
        try:
            ...
            if not processes:
                logger.warning(
                    ...
                    request_id=request.id,
                    response_preview=result_text[:200] if result_text else "N/A",
                )
                processes = [{
                    "name": "conversation",
            ...
            # Telemetry: Intent analysis complete with IR
            emit_telemetry(
                request_id=request.id,
                layer=3,
                layer_name="Prime Personality",
                ...
                operation="intent_analysis",
                status="complete",
                message=f"Intent: {ir.intent}, Processes: {len(ir.processes)}",
                session_id=request.session_id,
                payload={
                    "thought": {
                        "reasoning": f"Classified as '{ir.intent}' intent with {len(ir.processes)} execution processes",
                        "plan": [p.get("goal", "") for p in ir.processes],
                    }
                },
                metrics={
            ...
            logger.error(
                ...
                request_id=request.id,
                error=str(e),
                response=result_text[:500] if result_text else "N/A",
            )
            ...
            emit_telemetry(
                ...
                metrics={
                    "elapsed_ms": llm_latency_ms,
                    "llm_calls": 1 if result_text else 0,
                },
            )
